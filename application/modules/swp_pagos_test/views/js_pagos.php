...
<script type="text/javascript" >
    ...
    $(document).ready(function () {
        ...
        $("#txtAlumnoSearch").autocomplete({
            source: "<?= BASE_URL ?>swp_pagos_test/filtroAlumno",
            minLength: 2,
            select: function (event, ui) {
                ...
                if (ui.item.alucod != '') {
                    ...
                    if(ui.item.estado=='E' || ui.item.estado=='R') vestado =' - RETIRADO'; else vestado ='';
                    $("#txtAlumnoSearch").val(ui.item.nomcomp + " (" + $.trim(vnemosdes[2]) + vestado +")");
                    $("#txtAlumnoSearch").attr("disabled", true);
                   // $("#txtAlumnoSearch").css("font-weight","bold");
                  //  $("#txtAlumnoSearch").css("color","red");
                    $("#btnBuscar").focus();
                } else {
                    $('#htxtalumno').val('');
                    $('#htxtsalon').val('');
    ...
    function js_habilita(chk) {
        ...
        if (chk == true) {
            ...
            $('.input-number').each(function () {
                ...
                console.log("tip=" + tip + "*user=" + user);
                if (tip == '1') { // Pensiones
                    $("#" + id).removeAttr('disabled');
                }
                if (tip == '0' && (user == 'LHUAYTALLA' || user == 'MHUAYTALLA' || user == 'FHERRERA')) { // Matriculas
                    $("#" + id).removeAttr('disabled');
                }
            });
        } else {
    ...
    function js_registrarPago()
    {
        ...
        $.ajax({
            url: "<?= BASE_URL ?>swp_pagos_test/getPago/",
            type: "POST",
            dataType: "json",
                ...
                if (data['data'].length > 0) {
                    ...
                    $.each(data['data'], function (i, item) { //<li class="list-group-item nomargin">01 - PENSIÓN MARZO - 27-03-2019 =======>S/.320.00</li>
                        if (item.concob == '01') { // Para cuota de insripcion y matricula con pagos diferentes
                            ...
                            vChecks += ' <div style="float:right;"><input type="text" tp="0" value="' + parseInt(item.montopen) + '" class="input-number"  style="width: 40px;text-align:center;color:black;background-color: khaki;" disabled="disabled"  name="inputPagos[]" id="txtmonto_' + row + '" size="4"  onkeypress="return NumCheck(event, this);" onkeyup="js_recalcular(this.id, this.value);" maxlength="6" placeholder="00.00" /></div>';
                            vChecks += '</li>';
                        }
    ...
    function js_cargaApoderado(vfamcod) {
        ...
        //Ajax Load data from ajax
        $.ajax({
            url: "<?= BASE_URL ?>swp_pagos_test/getApoderados/",
            type: "POST",
            dataType: "json",
    ...
    function js_marcaApoderado(vdni, valor) {
        ...
        $.ajax({
            url: "<?= BASE_URL ?>swp_pagos_test/marcarApoderados/",
            type: "POST",
            dataType: "json",
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data.respuesta == 'ok') {
                        ...
                        // mandamos los nuevos datos a la pantalla principal
                        /*$("#txtdni").val(vdni);
                         $("#txtcliente").val(data.ap_paterno + " " + data.ap_materno + ", " + data.nombres);*/
                        // ==== Actualizamos los datos en la tabla familia 
                        if ($.trim(data.ap_paterno) == '' && $.trim(data.ap_materno) == '' && $.trim(data.nombres) == '') {
                            $("#txtdnisearch_" + row).val("");
                            vdni = "";
                        }
                        var dataFamilia = {
                            dni: vdni,
    ...
    function js_actualizaFamilia(data) {
        $.ajax({
            url: "<?= BASE_URL ?>swp_pagos_test/updateDatosApoderado/",
            type: "POST",
            dataType: "json",
            data: data,
            success: function (data)
    ...
    function js_delpago(vconcob, vmescob, valucod, numrecibo) {
        var usuario = $("#user").val();
        var msg = window.confirm("ESTA SEGURO DE ELIMINAR EL PAGO DEL MES : " + fn_meses(vmescob) + "?");
        if (msg) {
            if (usuario === 'LHUAYTALLA' || usuario === 'MHUAYTALLA' || usuario === 'FHERRERA') {
                var arrdata = {
                    vconcob: vconcob,
                    vmescob: vmescob,
                    valucod: valucod,
                    vnumrecibo: numrecibo
                };
                $.ajax({
                    url: "<?= BASE_URL ?>swp_pagos_test/deletePago/",
                    type: "POST",
                    dataType: "json",
                    data: arrdata,
                    success: function (data)
    ...
    function js_save() {
        ...
        if(vcbtipo>1){ // de tipo banco
            if($.trim(voucher)==""){
                alert("INGRESE EL NUMERO DE VOUCHER.");
                return false;                
            }
        }
        ...
        $.ajax({
            url: "<?= BASE_URL ?>swp_pagos_test/savePago/",
            type: "POST",
            dataType: "json",
            data: arrdata,
            success: function (data)
    ...
    function js_imprimir() {
        ...
        $("#formPrincipal").attr("action", "<?= BASE_URL ?>swp_pagos_test/printeecc/");
        $("#formPrincipal").submit();
    }

    function js_printBoleta(vBoleta, vtipo) {
        $("#rbdTipo").val(vtipo); // Boletas o Recibos
        $("#htxtnumrecibo").val(vBoleta);
        js_imprimirComprobantev2();
    }

    function js_envioBoleta(vBoleta, vtipo) {
        var email = window.prompt("INGRESE UN EMAIL VALIDO.", "");
        if (email != '' && email.length > 10) {
            alert("Mail Correcto pero Aún estamos en Implementacion");
        } else {
            var email = window.prompt("INGRESE UN EMAIL VALIDO.", "");
        }
    }
    ...
    function js_imprimirComprobantev2() {
        ...
        $("#formPrincipal").attr("action", "<?= BASE_URL ?>swp_pagos_test/printTicketV2/");
        $("#formPrincipal").submit();
    }
    ...
    function js_imprimirComprobante() {
        ...
        $("#formPrincipal").attr("action", "<?= BASE_URL ?>swp_pagos_test/printTicket/");
        $("#formPrincipal").submit();
    }
    
    function js_envioBoletaAdjunto(){
        var rbdTipo = $("#rbdTipo").val();
        var htxtsalon = $("#htxtsalon").val();
        var htxtalumno = $("#htxtalumno").val();
        var htxtnumrecibo = $("#htxtnumrecibo").val();
        $.ajax({
            url: "<?= BASE_URL ?>swp_pagos_test/printTicketEmail/",
            type: "POST",
            dataType: "json",
            data: {rbdTipo: rbdTipo,htxtsalon:htxtsalon, htxtalumno:htxtalumno, htxtnumrecibo:htxtnumrecibo },
            success: function (data)
            {
                console.log("BOLETA ENVIADA =>"+data);
            }
        });        
    }
    ...
    function js_save_concepto()
    {
         ...
         */
        $.ajax({
            url: "<?= BASE_URL ?>swp_pagos_test/grabarConcepto/",
            type: "POST",
            dataType: "json",
        ...
        $.ajax({
            ...
            success: function (data)
            {
                if (data['data'].length > 0) {
                    ...
                    $.each(data, function (i, row) {
                        var vimp = "";
                        if (row.tipo_comp == '01' || row.tipo_comp == '02' || row.tipo_comp == '03') { //  Recibos y Boletas
                            if (row.numrecibo == '0000000000') { // Recibos agregados manualmente
                                vimp = "";
                            } else {
                                vimp = "<a class='btn' title='Imprimir' onclick='js_printBoleta(\"" + row.numrecibo + "\",\"" + row.tipo_comp + "\")' ><i class='glyphicon glyphicon-print'></i></a>";
                                vimp += "<a class='btn' title='Envio por Correo' onclick='js_envioBoleta(\"" + row.numrecibo + "\",\"" + row.tipo_comp + "\")' ><i class='glyphicon glyphicon-envelope'></i></a>";
                            }
                        } /*else {
                         vimp = "&nbps;";
                         }*/
    ...
    function js_verPagos() {
        ...
        gridTable = $('#viewPagos').DataTable({
            ...
            "bLengthChange": false,
            "ajax": {
                "url": "<?= BASE_URL ?>swp_pagos_test/lstPagos/",
                "type": "POST",
