 ...
 */
class swp_pagos extends CI_Controller {
    ...
    public function adicional() {
        ...
        $this->load->view('view_lista_adiconal', $data);
        $this->load->view('view_footer');
    }
    ...
    public function getDocumento() {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            if ($vusu == ADMIN || $this->ano == 2018) {
                ...
                $cadGenerado = $this->objCobros->getGeneraNumero($vruc, $vTipo);
    ...
    public function getDocumentoAdicional() {
        $output = array();
        $flgArr = 0;
        if ($this->input->is_ajax_request()) {
            $vTipo = $this->input->post("vTipo");
            $vRazon = $this->input->post("vRazon");
            if ($vRazon == 'R01') {
                $vruc = RUC_PRIMARIA;
                $tipoRazon = '01';
            } else {
        ...
        if ($this->input->is_ajax_request()) {
            ...
            for ($x = 0; $x < count($arrDataMes); $x++) {
                if (($arrDataCob[$x] == '02') || ($arrDataCob[$x] == '01' && $vchkflag == '1')) {
                    $data = $this->objCobros->grabarPension($vIdAlu, $arrDataMes[$x], $arrDataCob[$x], $vnumrec, $arrDataPago[$i], $vcbtipo, $vfecha, $vcomp, $vruc, $vchkflag);
                    $i++;
                } else {
                    $data = $this->objCobros->grabarPension($vIdAlu, $arrDataMes[$x], $arrDataCob[$x], $vnumrec, 0, $vcbtipo, $vfecha, $vcomp, $vruc, $vchkflag);
                }
            }
            /* } */
            if ($data) {
    ...
    public function filtroAlumnoAll() {
        $returnArr = array();
        $txtFiltro = $this->input->get("term");
        $dataFiltro = $this->objCobros->filtroAlumnoAll($txtFiltro);
        foreach ($dataFiltro as $arrdata) {
            $alucod = $arrdata->ALUCOD;
            $rowArray['value'] = $alucod . " | " . $arrdata->NOMCOMP . " | " . $arrdata->NEMODES;
            $rowArray['alucod'] = $alucod;
            $rowArray['apepat'] = $arrdata->APEPAT;
            $rowArray['apemat'] = $arrdata->APEMAT;
            $rowArray['nombres'] = $arrdata->NOMBRES;
            $rowArray['nemo'] = $arrdata->NEMO;
            $rowArray['dni'] = $arrdata->DNI;
            $rowArray['nemodes'] = $arrdata->NEMODES;
            array_push($returnArr, $rowArray);
        }
        echo json_encode($returnArr);
    ...
    public function lstPagosAdicional() {
        if ($this->input->is_ajax_request()) {
            $output = array();
            $arrData = array();
            $data = $this->objCobros->get_datatables();
            if (is_array($data) && count($data) > 0) {
                foreach ($data as $fila) {
                    $conf = "<img src='" . base_url() . "img/delete.png' width='25px' heigth='25px' title='Eliminar Pago' onclick=\"javascript:js_eliminaPago('" . $fila->id . "','" . $fila->numrecibo . "');\" />";
                    $conf .= "<img src='" . base_url() . "images/imprimir.png' width='25px' heigth='25px' title='Imprimir Comprobante' onclick=\"javascript:js_reImprimirPago('" . $fila->numrecibo . "','" . $fila->tipo_razon . "');\" />";
                    $arrData [] = array(
                        "id" => $fila->id,
                        "fecreg" => $fila->fecreg,
                        "nomcomp" => ((strlen($fila->nomcomp)<8)?'SIN NOMBRE':$fila->nomcomp),
                        "concepto" => $fila->desconcepto,
                        "recibo" => $fila->numrecibo,
                        "monto" => 'S/' . $fila->monto,
                        "config" => ($fila->eliminado==1)? '<b>ELIMINADO</b>' : $conf
                    );
                }
                /*$output = array(
                ...
                $output = array(
                    "draw" => intval($_POST["draw"]),
                    "recordsTotal" => $this->objCobros->count_all(),
                    "recordsFiltered" => $this->objCobros->count_filtered(),
                    "data" => $arrData
                );             
    ...
    public function deletePagoAdicional() {
        $vid = $this->input->post("vid");
        $vnumrecibo = $this->input->post("vnumrecibo");
        $resp = $this->objCobros->deletePagoAdicional($vid);
        $vusu = $this->_session['USUCOD'];
        if ($resp) {
            if ($vusu != ADMIN) {
                $dataControl = array(
    ...
    public function getConceptos() {
        ...
        $output = array();
        $dataConceptos = $this->objCobros->getConceptos($idtipo);
        if (is_array($dataConceptos) && count($dataConceptos) > 0) {
            foreach ($dataConceptos as $concep) {
                $arrData [] = array(
                    'id' => $concep->idconcepto,
                    'value' => strtoupper($concep->desconcepto)
                );
            }
    ...
    public function grabarConceptoAdic() {
        ...
        try {
            ...
            $vrazon = $this->input->post("vrazon");
            $valucod = $this->input->post("valucod");
            $vtipocomp = $this->input->post("vtipocomp");
            $vmodalidad = $this->input->post("vmodalidad");
            $vvoucher = $this->input->post("vvoucher");
            $vdataItems = $this->input->post("dataItems");
            /* echo "<pre>";
              print_r($vdataItems);
              echo "</pre>";
              exit; */
            $tipopago =  ($vmodalidad=='1')  ? 'C' : 'B';         
            $vusu = $this->_session['USUCOD'];
            if ($valucod == "") {
                $valucod = $this->objCobros->generadorCodigo('ALU-EXTERNO', 'EX', 6);
                $vexterno = 1;
            } else {
                $vexterno = 0;
            }
            foreach ($vdataItems as $item) {
                $dataPost = array(
                    'anocob' => date('Y'),
                    'alucod' => $valucod,
                    'apepat' => $vapepat,
            ...
            $output = array('flg' => 1, 'msg' => 'OCURRIO UN ERROR AL REGISTRAR EL PAGO\nCOMUNIQUESE CON EL ADMINISTRADOR.');
        }
        echo json_encode($output);
    }
    ...
    public function printTicketV2() {
        ...
        $limite = 125 + $limite;
        $vusuPago = $this->objCobros->getDatoUsuarioPago($vcomp);
        ...
        $this->pdf->SetTitle('BOLETA ELECTRONICA - ' . $this->ano);
        ...
        $this->pdf->SetXY(8, 20);
        $this->pdf->Cell(30, 3, utf8_decode($vTipoDesc . ' ELECTRÓNICA'), 0, 0, 'C');
        $this->pdf->SetXY(8, 23);
        ...
        foreach ($dataPago as $pago) {
            ...
            $this->pdf->Cell(27, 3, utf8_decode(nombreConcepto($pago->concob) . ' - ' . nombreMesesCompleto($pago->mescob) . ' - ' . $this->ano), 'B', 0, 'L');
        ...
        $this->pdf->Cell(10, 3, utf8_decode("TOTAL: "), 0, 0, 'C');
    ...
    public function printTicket() {
        /* echo "<pre>";
          ...
          echo "</pre>";
         */
        // falta validar cuando primro imprimir con alucod y luego sin alucod .. se queda pegado el codigo 
        $vtipo = $_POST["rbdTipo"];  // "02";  // (tipo de documento)
        //$vNemo = $_POST["htxtsalon"]; // "2019034"; // (Codigo del Salon)
        $vAlumno = $_POST["htxtalumno"];  // "20170859"; // (Codigo del Alumno)
        $vcomp = $_POST["txtnumrecibo"]; // "B001-00000101"; (Numero de comprobante)
        $vusu = $this->_session['USUCOD']; // Codigo del Usuario
        $vrazon = $_POST["cbrazon"];
        $vidtipo = $_POST["cbtipo"];
        $vidconcepto = $_POST["cbconcepto"];
        $vmonto = $_POST["txtmontoConcepto"];

        $vpaterno = $_POST["txtpaterno"];
        $vmaterno = $_POST["txtmaterno"];
        $vnombres = $_POST["txtnombres"];
        $nomcomp = $vpaterno . ' ' . $vmaterno . ', ' . $vnombres;
        $rowConcepto = $this->objCobros->getConceptosxId($vidconcepto);
        $dataConcepto = $this->objCobros->getConceptosxComprobante($vcomp, $vrazon);
        if ($vrazon === 'R01') {
            $vruc = RUC_PRIMARIA;
        } else {
            $vruc = RUC_SECUNDARIA;
        ...
        $this->load->library('qrcodephp');
        /* RUC | TIPO DE DOCUMENTO | SERIE | NUMERO | MTO TOTAL IGV | MTO TOTAL DEL COMPROBANTE | FECHA DE EMISION |TIPO DE DOCUMENTO ADQUIRENTE | NUMERO DE DOCUMENTO ADQUIRENTE | */
        $textqr = $dataEmpresa[0]->ruc . '|01|' . $vcomp . '|' . $vmonto . '|' . date("d/m/Y") . '|' . $vtipo . '|';
        $rutaqr = $_SERVER['DOCUMENT_ROOT'] . "/intranet/img/qr/" . $vcomp . ".png";
        $rutaticket = base_url() . "img/qr/" . $vcomp . ".png";
        QRcode::png($textqr, $rutaqr, 'Q', 15, 0);
        // ================================================================
        ...
        $data['concepto'] = $rowConcepto->desconcepto;
        $data['dataconcepto'] = $dataConcepto;
    ...
    public function reimprimirTicket() {
        $vcomp = $_POST["txtnumero"];
        $vrazon = $_POST["txtrazon"];
        $dataConcepto = $this->objCobros->getConceptosxComprobante($vcomp, $vrazon);
        $vusu =  $dataConcepto[0]->usureg;
        $vrazon = $dataConcepto[0]->tipo_razon;
        $vmonto = $dataConcepto[0]->monto;
        $vpaterno = $dataConcepto[0]->apepat;
        $vmaterno = $dataConcepto[0]->apemat;
        $vnombres = $dataConcepto[0]->nombres;
        $vtipo = $dataConcepto[0]->tipo_comp;
        $fecha = $dataConcepto[0]->fecreg;
        $nomcomp = $vpaterno . ' ' . $vmaterno . ', ' . $vnombres;
    ...
    public function printcpv() {
        ...
        $this->pdf->Cell(50, 5, utf8_decode('Teléfono : ' . $dataEmpresa[0]->telefono), 0, 0, 'C');
